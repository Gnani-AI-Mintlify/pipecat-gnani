...
- **Bumped `gnani-vachana` dependency to `>=0.7.9,<1.0`** — picks up `ogg` container support, OGG/Opus at all sample rates, WAV-wrapping fix for non-PCM formats, `SUPPORTED_SAMPLE_RATES` export, and `AudioConfig` default `sample_rate=48000`.

## [0.5.10] - 2026-07-17

### Changed

- **Bumped `gnani-vachana` dependency to `>=0.7.8,<1.0`** — picks up optional TTS `speed` support in the core SDK. This plugin does not expose or send `speed`; TTS payloads are unchanged.
